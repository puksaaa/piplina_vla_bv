...
CAMERA_BASE_URL = os.getenv("CAMERA_BASE_URL", "http://100.64.0.1:8090").rstrip("/")
...
def robot_run(payload: RobotPlanRequest) -> dict[str, Any]:
    if ORCHESTRATOR_ENABLED:
        if payload.images:
            raise HTTPException(
                status_code=400,
                detail="Orchestrated runs use fresh camera frames. Remove uploaded images and retry.",
            )
...
@app.post("/api/robot/runs/{run_id}/approve")
def approve_robot_run(run_id: str) -> dict[str, Any]:
    if not ORCHESTRATOR_ENABLED:
        raise HTTPException(status_code=404, detail="Orchestrator mode is disabled.")
    return orchestrator_request("POST", f"/v1/runs/{run_id}/approve")
...
def start_orchestrated_run(command: str) -> dict[str, Any]:
    ...
    if not isinstance(run_id, str) or not run_id:
        raise HTTPException(status_code=502, detail="Orchestrator returned no run identifier.")
...
# Also expose the design assets at the app root. This keeps the standalone
# index.html preview and the FastAPI-served version using the same relative URLs.
