...
USB-камеры открывает только `camera_splitter.py`. SmolVLA получает кадры через ZMQ-разветвитель, а VLM/Gemma получает HTTP-снимки. Резидентный runner держит policy в CUDA и отдаёт численные action chunks только во внешнюю защищённую границу исполнения: сам этот проект не открывает serial-порт и не передаёт моторные команды.
...
Runner держит SmolVLA в CUDA-памяти один раз. Он принимает action/revision от supervisor, но сам не открывает порт робота и не отправляет моторные команды.
